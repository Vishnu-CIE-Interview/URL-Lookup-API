...
Every time an URL lookup request is recieved, the API server will check the Memcached server to examine if a cached categorization value is available for the URL in question. If a cached value is not available, the API server forwards this lookup request to the MySQL database server. Once a categorization is obtained from the database, the values are immediately populated in the caching layer, so that on any subsequent lookup requests for the same URL, a database lookup need not be made. Every entry in the cache layer has a Time-To-Live (TTL) value which decides when the value in the cache will be invalidated. This is important because, we do not want to provide the requesting client with URL lookup categories that are stale, for which new categorization is available on the database. 

The current time to live is set as 10 minutes, as this is the incoming rate of new URL categorization that is ingested on the database. This ensures that each value in the cache is invalidated after 10 minutes, and on subsequent client queries, the new values from the database can get re-populated in the cache. 

The Memcache Caching Layer can be scaled up further to be a system of distributed machines, thereby improving the amount of caching information it can store. The existing design ensure that such a future scaling can be easily incorporated into the existing system.
